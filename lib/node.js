	...
	try {
		...
		axios.get('http://' + (process.env.RPC_HOST || 'localhost') + ':8669/staking/candidates')
			.then(function (response) {
				...
				vm.info.name = current_node.name;
				vm.info.contact = current_node.description;
				vm.info.node = (current_node.totalVotes > 0) ? ("Validator") : ("Full node");
...
Node.prototype.getTotalVotes = function () {
	...
	axios.get('http://' + (process.env.RPC_HOST || 'localhost') + ':8669/staking/candidates')
		.then(function (response) {
			let current_node = response.data.find(n => n.ipAddr == (process.env.RPC_HOST || 'localhost'));
			let voting_power = (parseInt(current_node.totalVotes) / 1000000);
			vm.stats.hashrate = voting_power;
		})
		.catch(function (error) {
			// handle error
			vm.stats.hashrate = 0;
