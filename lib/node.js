...
const CONNECTION_ATTEMPTS_TIMEOUT = 500;
	...
	if (!this._web3) {
		if (web3.isConnected()) {
			...
			if (this._connection_attempts < MAX_CONNECTION_ATTEMPTS) {
				console.error('Web3 connection attempt', chalk.cyan('#' + this._connection_attempts++), 'failed. Are you sure port 8545 is open?');
				console.error('Trying again in', chalk.cyan(CONNECTION_ATTEMPTS_TIMEOUT * this._connection_attempts + ' ms'));
...
Node.prototype.getInfo = function () {
	...
	try {
		this.info.coinbase = "mtr";
		this.info.node = "Meter Tesla";
		this.info.net = "mainnet";
		this.info.protocol = "0.0.5";
		this.info.api = "1.0.0";
...
Node.prototype.getCurrentPeerCount = function () {
	let self = this;
	axios.get('http://' + (process.env.RPC_HOST || 'localhost') + ':8669/node/network/peers')
		.then(function (response) {
			let peerCount = response.data.length;
			console.info('Current peer count: '+ peerCount)
			self.stats.peers = peerCount;
		})
		.catch(function (error) {
			// handle error
			self.stats.peers = 0;
			console.error("Unable to retrieve peers")
			console.error(error);
		})
}
